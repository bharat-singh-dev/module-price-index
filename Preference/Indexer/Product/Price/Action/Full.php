...
namespace SixtySeven\PriceIndex\Preference\Indexer\Product\Price\Action;
...
class Full extends \Magento\Catalog\Model\Indexer\Product\Price\Action\Full
{
        ...
        try { 
            ...
            foreach ($this->getTypeIndexers() as $indexer) {
                ...
                $batches = $this->batchProvider->getBatches(
                    ...
                    $this->batchSizeCalculator->estimateBatchSize($connection, $indexer->getTypeId()),$indexer->getTypeId()
                );
                $entityIdVals = [];
                foreach ($batches as $batch) {
                    ...
                    $entityIdVals = array_merge($entityIdVals, $entityIds);
                    if (!$indexer->getIsComposite()) {
                        $this->executeBatchProcess($connection, $indexer, $replicaTable, $entityIds);
                    }
                }

                if ($indexer->getIsComposite()) {
                    $this->executeBatchProcess($connection, $indexer, $replicaTable, $entityIdVals);
                }
            }
    ...
    private function executeBatchProcess($connection, $indexer, $replicaTable, $entityIds)
    {
        if (!empty($entityIds)) {
